...
function applyMetaToBookManifest(book, meta) {
  var bookMeta = meta.books.filter(bookMeta => bookMeta.name === book.code)[0];
  Object.assign(book, bookMeta);
}

function applyMetaToManifest(books, meta) {
  meta.books.forEach(bookMeta => {
    var book = books.names[bookMeta.name];
    Object.assign(book, bookMeta);
  });
}
...
function parseBook(book, html) {
  ...
  //var $ = cheerio.load(dom.window.document);
  var $ = cheerio.load(html);
  var $chapters = $("select[name=chapter]");
  var chapters = [];
  $chapters.children("option").each((i, option) => {
    var $option = $(option);
...
function loadBookManifest(book) {
  ...
  return loadLocalFile("books/" + code + ".json").then(json => JSON.parse(json)).catch(() => loadFile("books/" + code + ".html", {
    url: URLOpt(Object.assign({
      path: path.match(/^\/(.+)/)[1].split('\/')
    }, domain))
  }).then(html => {
    fs.writeFile("chapters/" + code + "-001.html", html, "utf8", err => {
      if (err)
        console.log(err);
    });
    return html;
